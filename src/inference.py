...
Equivocal zone mirrors R's adjust_equivocal_zone(threshold=0.7, value=0.2):
  - prob > 0.7  → "Contaminated"
  - prob in [0.5, 0.7] → "Equivocal"
  - prob < 0.5  → "Real"
"""
...
_CONTAM_THRESHOLD = 0.7
_EQUIVOCAL_LOW = 0.5   # threshold - value = 0.7 - 0.2
...
def _predict_proba_1(pipeline, X: pd.DataFrame) -> np.ndarray:
    ...
    # proba shape: (n, 2) where column 1 = P(contaminated)
    if proba.ndim == 2:
        # Find the index of class label 1
...
def make_bmp_predictions(
    ...
    for fluid in fluids:
        for timing in timings:
            ...
            if len(valid_idx) > 0:
                ...
                for sl in _chunk_indices(len(X_valid), chunk_size):
                    prob_chunks.append(_predict_proba_1(pipeline, X_valid.iloc[sl]))
                valid_probs = np.concatenate(prob_chunks).round(3)
                probs[valid_idx] = valid_probs
                preds[valid_idx] = [label_pred_class(p) for p in valid_probs]
        ...
        if mix_dict is not None:
            ...
            if len(valid_idx) > 0:
                ...
                ratio_chunks = []
                for sl in _chunk_indices(len(X_valid), chunk_size):
                    ratio_chunks.append(pipeline.predict(X_valid.iloc[sl]))
                ratios[valid_idx] = np.concatenate(ratio_chunks).round(3)
            mix_cols[f"mix_ratio_{fluid}"] = ratios
    ...
    # Aggregate columns: any fluid (excluding LR) flagged as Contaminated?
    ...
    # Max prob across fluids (excluding LR)

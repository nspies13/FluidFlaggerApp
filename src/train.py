...
def _make_classifier(n_rows: int, X, y, n_jobs: int = 1) -> tuple:
    ...
    use_lgbm = n_rows >= _RF_THRESHOLD and _LGBM_AVAILABLE
...
def save_models(
    ...
    """Save model dicts as .joblib or .pkl files. cv_metrics are stripped so
    the saved object contains only the pipeline and its identifying metadata.
    ...
    paths = []
    for m in models:
        payload = {k: v for k, v in m.items() if k != "cv_metrics"}
        key = model_key(m)
        if fmt == "pkl":

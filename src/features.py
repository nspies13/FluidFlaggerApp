...
def _add_pre_post(df: pd.DataFrame, analytes: list[str], window_hours: float) -> pd.DataFrame:
...
def add_pre_post_bmp(df: pd.DataFrame, lookback_hours: float = 48.0) -> pd.DataFrame:
    ...
    return _add_pre_post(df, BMP_ANALYTES, lookback_hours)
...
def add_pre_post_cbc(df: pd.DataFrame, collection_interval: float = 48.0) -> pd.DataFrame:
    ...
    return _add_pre_post(df, CBC_ANALYTES, collection_interval)
...
class _PanelFeatureTransformer(BaseEstimator, TransformerMixin):
    """
    Base transformer for panel (BMP or CBC) log-delta + PCA features.

    Subclasses set `_analytes` and `_realtime_prior_pca` (whether realtime mode
    fits a separate prior_pca_ — True for BMP, False for CBC).

    mode='realtime':
        Base features: N current + N prior_deltas + n_components all_PC
        BMP also appends: n_components prior_PC

    mode='retrospective':
        Features: N current + N prior_deltas + N post_deltas
                  + n_components prior_PC + n_components post_PC + n_components all_PC
    """

    _analytes: list[str] = []
    _realtime_prior_pca: bool = False
    ...
    def _compute_deltas(self, X: pd.DataFrame):
        prior_deltas = np.column_stack([
            _log_delta_prior(X[c].values.astype(float), X[f"{c}_prior"].values.astype(float))
            for c in self._analytes
        ])
        if self.mode == "retrospective":
            post_deltas = np.column_stack([
                _log_delta_post(X[c].values.astype(float), X[f"{c}_post"].values.astype(float))
                for c in self._analytes
            ])
    ...
    def fit(self, X: pd.DataFrame, y=None):
        X = pd.DataFrame(X)
        current = X[self._analytes].values.astype(float)
    ...
    def transform(self, X: pd.DataFrame):
        ...
        if self.mode == "retrospective":
            ...
            arr = np.hstack([current, prior_deltas, post_deltas, prior_pcs, post_pcs, all_pcs])
        else:
            all_predictors = np.hstack([current, prior_deltas])
    ...
    def get_feature_names_out(self, input_features=None):
        a = self._analytes
        k = self.n_components
        pcs = [f"prior_PC{i+1}" for i in range(k)]
        if self.mode == "retrospective":
            names = (
                a
                + [f"{c}_log_delta_prior" for c in a]
                + [f"{c}_log_delta_post" for c in a]
                + pcs
                + [f"post_PC{i+1}" for i in range(k)]
                + [f"all_PC{i+1}" for i in range(k)]
            )
        else:
            names = a + [f"{c}_log_delta_prior" for c in a] + [f"all_PC{i+1}" for i in range(k)]
            if self._realtime_prior_pca:
                names = names + pcs
        return np.array(names)


class BMPFeatureTransformer(_PanelFeatureTransformer):
    """
    Transform BMP DataFrame rows into model-ready feature matrix.

    mode='realtime':  8 current + 8 prior_deltas + 3 all_PC + 3 prior_PC = 22
    mode='retrospective':  8 current + 8 prior_deltas + 8 post_deltas
                           + 3 prior_PC + 3 post_PC + 3 all_PC = 33
    """
    _analytes = BMP_ANALYTES
    _realtime_prior_pca = True


class CBCFeatureTransformer(_PanelFeatureTransformer):
    """
    Transform CBC DataFrame rows into model-ready feature matrix.

    mode='realtime':  3 current + 3 prior_deltas + 3 all_PC = 9
    mode='retrospective':  3 current + 3 prior_deltas + 3 post_deltas
                           + 3 prior_PC + 3 post_PC + 3 all_PC = 18
    """
    _analytes = CBC_ANALYTES
    _realtime_prior_pca = False
